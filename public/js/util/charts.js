...
// Read live from the CSS custom properties rather than hardcoding hex, so
// the charts follow both the active palette and the light/dark theme
// instead of staying frozen on whatever color was baked in at write time.
...
function chartPalette() {
  return {
    mark: themeColor("--color-primary", "#6c56e0"),
    axis: themeColor("--border", "#e2dcf3"),
    axisText: themeColor("--color-text-muted", "#6c6684"),
  };
}
...
export function renderBarChart({ labels, values, height = 220 }) {
  const { mark, axis, axisText } = chartPalette();
  ...
  const gridLines = [0, 0.5, 1].map((t) => {
    ...
    return `
      ...
      <text x="${padLeft - 8}" y="${y + 4}" text-anchor="end" font-size="10" fill="${axisText}">${val}</text>
    `;
  }).join("");
  ...
  const bars = labels.map((label, i) => {
    ...
    return `
      <rect class="chart-bar chart-bar--grow-v" style="animation-delay:${i * 45}ms;" x="${x}" y="${y}" width="${barW}" height="${Math.max(barH, 1)}" rx="4"
        fill="${mark}" data-label="${escapeHtml(label)}" data-value="${v}" />
      <text x="${x + barW / 2}" y="${height - 8}" text-anchor="middle" font-size="10" fill="${axisText}">${escapeHtml(label)}</text>
    `;
  }).join("");
...
export function renderHBarChart({ labels, values, rowHeight = 34 }) {
  const { mark, axisText } = chartPalette();
  const width = 460;
  ...
  const rows = labels.map((label, i) => {
    ...
    return `
      <text x="${labelW - 10}" y="${y + rowHeight / 2 + 4}" text-anchor="end" font-size="12" fill="var(--ink-700, #423c59)" font-weight="600">${escapeHtml(label)}</text>
      <rect class="chart-bar chart-bar--grow-h" style="animation-delay:${i * 45}ms;" x="${labelW}" y="${y}" width="${Math.max(barW, 2)}" height="${rowHeight - 12}" rx="4"
        fill="${mark}" data-label="${escapeHtml(label)}" data-value="${v}" />
      <text x="${labelW + barW + 8}" y="${y + rowHeight / 2 - 6 + 4}" font-size="11" fill="${axisText}">${v}</text>
    `;
  }).join("");
